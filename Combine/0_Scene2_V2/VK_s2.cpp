...
// ---- Alias old globals to Scene2 state (drop this right after `scene2_struct s2;`) ----
#define enableInstanceExtensionCount          (s2.enableInstanceExtensionCount)
#define enabledInstanceExtensionNames_array   (s2.enabledInstanceExtensionNames_array)

#define vkInstance                            (s2.vkInstance)
#define vkSurfaceKHR                          (s2.vkSurfaceKHR)

#define vkPhysicalDevice_selected             (s2.vkPhysicalDevice_selected)
#define graphicsQueueFamilyIndex_selected     (s2.graphicsQueueFamilyIndex_selected)
#define vkPhysicalDeviceMemoryProperties      (s2.vkPhysicalDeviceMemoryProperties)
#define physicalDeviceCount                   (s2.physicalDeviceCount)
#define vkPhysicalDevice_array                (s2.vkPhysicalDevice_array)

#define enabledDeviceExtensionCount           (s2.enabledDeviceExtensionCount)
#define enabledDeviceExtensionNames_array     (s2.enabledDeviceExtensionNames_array)

#define vkDevice                              (s2.vkDevice)
#define vkQueue                               (s2.vkQueue)

#define vkDescriptorSet                       (s2.vkDescriptorSet)

#define vkFormat_color                        (s2.vkFormat_color)
#define vkColorSpaceKHR                       (s2.vkColorSpaceKHR)
#define vkPresentModeKHR                      (s2.vkPresentModeKHR)

#define winWidth                              (s2.winWidth)
#define winHeight                             (s2.winHeight)
#define vkSwapchainKHR                        (s2.vkSwapchainKHR)
#define vkExtent2D_swapchain                  (s2.vkExtent2D_swapchain)

#define swapchainImageCount                   (s2.swapchainImageCount)
#define swapchainImage_array                  (s2.swapchainImage_array)
#define swapchainImageView_array              (s2.swapchainImageView_array)

#define vkFormat_depth                        (s2.vkFormat_depth)
#define vkImage_depth                         (s2.vkImage_depth)
#define vkDeviceMemory_depth                  (s2.vkDeviceMemory_depth)
#define vkImageView_depth                     (s2.vkImageView_depth)

#define vkCommandPool                         (s2.vkCommandPool)
#define vkCommandBuffer_array                 (s2.vkCommandBuffer_array)

#define vkRenderPass                          (s2.vkRenderPass)
#define vkFramebuffer_array                   (s2.vkFramebuffer_array)

#define vkSemaphore_backBuffer                (s2.vkSemaphore_backBuffer)
#define vkSemaphore_renderComplete            (s2.vkSemaphore_renderComplete)
#define vkFence_array                         (s2.vkFence_array)

#define vkClearColorValue                     (s2.vkClearColorValue)
#define vkClearDepthStencilValue              (s2.vkClearDepthStencilValue)

#define bInitialized                          (s2.bInitialized)
#define currentImageIndex                     (s2.currentImageIndex)

#define bValidation                           (s2.bValidation)
#define enabledValidationLayerCount           (s2.enabledValidationLayerCount)
#define enabledValidationLayerNames_array     (s2.enabledValidationLayerNames_array)
#define vkDebugReportCallbackEXT              (s2.vkDebugReportCallbackEXT)
#define vkDestroyDebugReportCallbackEXT_fnptr (s2.vkDestroyDebugReportCallbackEXT_fnptr)

// Overlays packed into Scene2 state:
#define vkImage_overlay                       (s2.vkImage_overlay)
#define vkDeviceMemory_overlay                (s2.vkDeviceMemory_overlay)
#define vkImageView_overlay                   (s2.vkImageView_overlay)
#define vkSampler_overlay                     (s2.vkSampler_overlay)

#define gOverlayImages                        (s2.gOverlayImages)
#define gOverlayMem                           (s2.gOverlayMem)
#define gOverlayViews                         (s2.gOverlayViews)
#define gOverlaySamplers                      (s2.gOverlaySamplers)

// Descriptor-change re-record flag lives with Scene2:
#define gCmdBuffersDirty                      (s2.gCmdBuffersDirty)
...
VkResult createShaders(void){
	...
	// Code
	// For vertex shader
	const char* szFileName = "shader_s2.vert.spv";
	FILE* fp = NULL;
	size_t size;
