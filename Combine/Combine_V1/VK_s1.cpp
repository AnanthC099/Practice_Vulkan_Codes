...
// ===== Subpass 1 : Scene 2 (from VK_s2.cpp) – resources =====
struct Scene2_Resources
{
    // --- Shaders ---
    ...
    struct UBO {
        ...
        glm::mat4 viewMatrix;
        glm::mat4 projectionMatrix;
        glm::vec4 overlayParams; // x=fade, y=screenW, z=screenH, w=sizeFrac
    } uboHost;
    ...
    // --- Graphics pipeline targeting subpass #1 ---
    VkPipeline pipeline = VK_NULL_HANDLE;

    // --- Skybox cubemap (same face order as VK_s2.cpp) ---
    VkImage        cubeImg = VK_NULL_HANDLE;
    VkDeviceMemory cubeMem = VK_NULL_HANDLE;
    VkImageView    cubeView = VK_NULL_HANDLE;
    VkSampler      cubeSampler = VK_NULL_HANDLE;

    // --- Overlay gallery (optional; first one is bound initially) ---
    static const int kOverlayCount = 12;
    VkImage        ovImg    [kOverlayCount] = {};
    VkDeviceMemory ovMem    [kOverlayCount] = {};
    VkImageView    ovView   [kOverlayCount] = {};
    VkSampler      ovSampler[kOverlayCount] = {};
    int            ovBound = -1;
} gS2;

// --- Scene 2 (Subpass 1) setup ---
VkResult S2_CreateShaders();
VkResult S2_CreateDescriptorSetLayout();
VkResult S2_CreatePipelineLayout();
VkResult S2_CreateDescriptorPool();
VkResult S2_CreateUniformBuffer();
VkResult S2_UpdateUniformBuffer();
VkResult S2_CreateCubemap(const char* faces[6]);      // loads right/left/top/bottom/front/back
VkResult S2_CreateOverlay2D(const char* path,
                            VkImage* outImg, VkDeviceMemory* outMem,
                            VkImageView* outView, VkSampler* outSampler);
VkResult S2_CreateDescriptorSet();                     // binds UBO + cubemap + overlay#0
VkResult S2_CreatePipeline();                          // subpass = 1

// Recording helper used inside buildCommandBuffers()
static void S2_RecordSubpass1(VkCommandBuffer cb);

// Optional: change overlay at runtime (binds into descriptor set binding=2)
void S2_BindOverlay(int index);

// ---------- Scene 2 (Subpass 1) : Shaders ----------
VkResult S2_CreateShaders()
{
    ...
    // Vertex
    {
        const char* fn = "shader_s2.vert.spv";
        FILE* fp = fopen(fn, "rb");
        if(!fp) { fprintf(gpFile, "S2_CreateShaders(): fopen failed for %s\n", fn); return VK_ERROR_INITIALIZATION_FAILED; }
        fseek(fp,0,SEEK_END); size_t sz = ftell(fp); fseek(fp,0,SEEK_SET);
        std::vector<uint8_t> bytes(sz);
        fread(bytes.data(), 1, sz, fp); fclose(fp);

        VkShaderModuleCreateInfo ci{ VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO };
        ci.codeSize = sz; ci.pCode = reinterpret_cast<const uint32_t*>(bytes.data());
        vr = vkCreateShaderModule(s1.vkDevice, &ci, nullptr, &gS2.vs);
        if(vr) return vr;
    }
    // Fragment
    {
        const char* fn = "shader_s2.frag.spv";
        FILE* fp = fopen(fn, "rb");
        if(!fp) { fprintf(gpFile, "S2_CreateShaders(): fopen failed for %s\n", fn); return VK_ERROR_INITIALIZATION_FAILED; }
        fseek(fp,0,SEEK_END); size_t sz = ftell(fp); fseek(fp,0,SEEK_SET);
        std::vector<uint8_t> bytes(sz);
        ...
        VkShaderModuleCreateInfo ci{ VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO };
        ci.codeSize = sz; ci.pCode = reinterpret_cast<const uint32_t*>(bytes.data());
        vr = vkCreateShaderModule(s1.vkDevice, &ci, nullptr, &gS2.fs);
        if(vr) return vr;
    }
    fprintf(gpFile, "S2 shaders created (s2.vert/s2.frag)\n");
    return vr;
}

// ---------- Descriptor set layout (UBO + cubemap + overlay2D) ----------
VkResult S2_CreateDescriptorSetLayout()
{
    ...
    return vkCreateDescriptorSetLayout(s1.vkDevice, &ci, nullptr, &gS2.setLayout);
}

VkResult S2_CreatePipelineLayout()
{
    VkPipelineLayoutCreateInfo ci{ VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO };
    ci.setLayoutCount = 1; ci.pSetLayouts = &gS2.setLayout;
    return vkCreatePipelineLayout(s1.vkDevice, &ci, nullptr, &gS2.pipelineLayout);
}
...
VkResult S2_CreateUniformBuffer()
{
    ...
    bi.size = sizeof(Scene2_Resources::UBO);
    bi.usage = VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT;

    vr = vkCreateBuffer(s1.vkDevice, &bi, nullptr, &gS2.ubo); if(vr) return vr;

    VkMemoryRequirements mr{}; vkGetBufferMemoryRequirements(s1.vkDevice, gS2.ubo, &mr);
    ...
    for(uint32_t i=0;i<s1.vkPhysicalDeviceMemoryProperties.memoryTypeCount;++i){
        if((bits & 1) && (s1.vkPhysicalDeviceMemoryProperties.memoryTypes[i].propertyFlags &
            (VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT|VK_MEMORY_PROPERTY_HOST_COHERENT_BIT)))
        { ai.memoryTypeIndex = i; break; }
        bits >>= 1;
    }
    vr = vkAllocateMemory(s1.vkDevice, &ai, nullptr, &gS2.uboMem); if(vr) return vr;
    vr = vkBindBufferMemory(s1.vkDevice, gS2.ubo, gS2.uboMem, 0);  if(vr) return vr;
    return S2_UpdateUniformBuffer();
}

VkResult S2_UpdateUniformBuffer()
{
    // Simple skybox matrices (camera at origin); match VK conventions
    gS2.uboHost.modelMatrix = glm::mat4(1.0f);
    gS2.uboHost.viewMatrix  = glm::mat4(1.0f); // skybox usually uses only orientation; start with identity
    glm::mat4 P = glm::perspective(glm::radians(45.0f),
                                   (float)s1.winWidth/(float)s1.winHeight,
                                   0.1f, 100.0f);
    P[1][1] *= -1.0f; // Vulkan Y
    gS2.uboHost.projectionMatrix = P;

    // Overlay params: start hidden, size = 0.85 of min dimension (like VK_s2.cpp)
    gS2.uboHost.overlayParams = glm::vec4(0.0f, (float)s1.winWidth, (float)s1.winHeight, 0.85f);

    void* p=nullptr;
    VkResult vr = vkMapMemory(s1.vkDevice, gS2.uboMem, 0, sizeof(gS2.uboHost), 0, &p);
    if(vr==VK_SUCCESS){ memcpy(p, &gS2.uboHost, sizeof(gS2.uboHost)); vkUnmapMemory(s1.vkDevice, gS2.uboMem); }
    return vr;
}



// ---------- Tiny helper to make a single 2D texture (used for overlays) ----------
VkResult S2_CreateOverlay2D(const char* path,
                            VkImage* outImg, VkDeviceMemory* outMem,
                            VkImageView* outView, VkSampler* outSampler)
{
	VkResult createTexture2D(const char* path,
                         VkImage* outImg, VkDeviceMemory* outMem,
                         VkImageView* outView, VkSampler* outSampler);
    // This is the same flow you use in VK_s2.cpp:createTexture2D(); kept compact
    return createTexture2D(path, outImg, outMem, outView, outSampler); // reuse your existing helper if it's in this TU
    // If not available in VK_s1.cpp, copy the short helper from VK_s2.cpp here verbatim. :contentReference[oaicite:1]{index=1}
}

// ---------- Cubemap (6 faces) : pure C, no externs ----------
VkResult S2_CreateCubemap(const char* faces[6])
{
    // faces order expected: +X, -X, +Y, -Y, +Z, -Z  (right, left, top, bottom, front, back)
    // We require all six to be provided and of identical size.
    VkResult vr = VK_SUCCESS;

    if (!faces) {
        fprintf(gpFile, "S2_CreateCubemap(): faces==NULL, falling back to 2D texture handle\n");
        gS2.cubeImg = VK_NULL_HANDLE; gS2.cubeMem = VK_NULL_HANDLE;
        gS2.cubeView = s1.vkImageView_texture;  // fallback (works only if shader expects sampler2D)
        gS2.cubeSampler = s1.vkSampler_texture;
        return VK_SUCCESS;
    }

    // 1) Load 6 images with stb_image
    int W = 0, H = 0, C = 0;
    stbi_uc* pix[6] = {0};
    for (int i = 0; i < 6; ++i) {
        if (!faces[i]) { fprintf(gpFile, "S2_CreateCubemap(): faces[%d] is NULL\n", i); vr = VK_ERROR_INITIALIZATION_FAILED; goto FAIL_LOAD; }
        pix[i] = stbi_load(faces[i], &W, &H, &C, STBI_rgb_alpha);
        if (!pix[i] || W <= 0 || H <= 0) { fprintf(gpFile, "S2_CreateCubemap(): stbi_load failed for %s\n", faces[i]); vr = VK_ERROR_INITIALIZATION_FAILED; goto FAIL_LOAD; }
        if (i == 0) { /* lock W,H */ }
        else {
            int w2,h2,ch2;
            stbi_info(faces[i], &w2, &h2, &ch2); // (best-effort) size check
            if (w2 != W || h2 != H) { fprintf(gpFile, "S2_CreateCubemap(): face size mismatch at %d (%s)\n", i, faces[i]); vr = VK_ERROR_INITIALIZATION_FAILED; goto FAIL_LOAD; }
        }
    }

    const VkDeviceSize faceSize = (VkDeviceSize)W * (VkDeviceSize)H * 4; // RGBA8
    const VkDeviceSize totalSize = faceSize * 6;

    // 2) Create staging buffer and upload all 6 faces contiguously
...

    {
        ...
        vr = vkCreateBuffer(s1.vkDevice, &ci, NULL, &staging);
        if (vr) { fprintf(gpFile, "S2_CreateCubemap(): vkCreateBuffer(staging) failed %d\n", vr); goto FAIL; }

        VkMemoryRequirements mr;
        memset(&mr, 0, sizeof(mr));
        vkGetBufferMemoryRequirements(s1.vkDevice, staging, &mr);
        ...
        for (uint32_t i = 0; i < s1.vkPhysicalDeviceMemoryProperties.memoryTypeCount; ++i) {
            if ((bits & 1u) &&
                (s1.vkPhysicalDeviceMemoryProperties.memoryTypes[i].propertyFlags &
                 (VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT)) ==
                 (VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT)) {
                ai.memoryTypeIndex = i; break;
            }
        ...
        vr = vkAllocateMemory(s1.vkDevice, &ai, NULL, &stagingMem);
        if (vr) { fprintf(gpFile, "S2_CreateCubemap(): vkAllocateMemory(staging) failed %d\n", vr); goto FAIL; }
        vr = vkBindBufferMemory(s1.vkDevice, staging, stagingMem, 0);
        if (vr) { fprintf(gpFile, "S2_CreateCubemap(): vkBindBufferMemory(staging) failed %d\n", vr); goto FAIL; }

        void* p = NULL;
        vr = vkMapMemory(s1.vkDevice, stagingMem, 0, totalSize, 0, &p);
        if (vr) { fprintf(gpFile, "S2_CreateCubemap(): vkMapMemory(staging) failed %d\n", vr); goto FAIL; }

        // Copy each face sequentially
        for (int i = 0; i < 6; ++i) {
            memcpy((uint8_t*)p + faceSize * i, pix[i], (size_t)faceSize);
        }
        vkUnmapMemory(s1.vkDevice, stagingMem);
    }

    // 3) Create cube image (arrayLayers=6, cube-compatible)
    {
        ...
        vr = vkCreateImage(s1.vkDevice, &ci, NULL, &gS2.cubeImg);
        if (vr) { fprintf(gpFile, "S2_CreateCubemap(): vkCreateImage(cube) failed %d\n", vr); goto FAIL; }

        VkMemoryRequirements mr;
        memset(&mr, 0, sizeof(mr));
        vkGetImageMemoryRequirements(s1.vkDevice, gS2.cubeImg, &mr);
        ...
        for (uint32_t i = 0; i < s1.vkPhysicalDeviceMemoryProperties.memoryTypeCount; ++i) {
            if ((bits & 1u) &&
                (s1.vkPhysicalDeviceMemoryProperties.memoryTypes[i].propertyFlags & VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT)) {
                ai.memoryTypeIndex = i; break;
            }
            bits >>= 1u;
        }

        vr = vkAllocateMemory(s1.vkDevice, &ai, NULL, &gS2.cubeMem);
        if (vr) { fprintf(gpFile, "S2_CreateCubemap(): vkAllocateMemory(cube) failed %d\n", vr); goto FAIL; }
        vr = vkBindImageMemory(s1.vkDevice, gS2.cubeImg, gS2.cubeMem, 0);
        if (vr) { fprintf(gpFile, "S2_CreateCubemap(): vkBindImageMemory(cube) failed %d\n", vr); goto FAIL; }
    }

    // 4) Transition UNDEFINED -> TRANSFER_DST for all 6 layers & copy each face
    {
        // One-time CB
        VkCommandBufferAllocateInfo a;
        memset(&a, 0, sizeof(a));
        a.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
        a.commandPool = s1.vkCommandPool;
        a.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
        a.commandBufferCount = 1;
        VkCommandBuffer cb = VK_NULL_HANDLE;
        vr = vkAllocateCommandBuffers(s1.vkDevice, &a, &cb);
        if (vr) { fprintf(gpFile, "S2_CreateCubemap(): vkAllocateCommandBuffers() failed %d\n", vr); goto FAIL; }
        ...
        for (uint32_t i = 0; i < 6u; ++i) {
            ...
            regions[i].imageExtent.width  = (uint32_t)W;
            regions[i].imageExtent.height = (uint32_t)H;
            regions[i].imageExtent.depth  = 1;
        }

        vkCmdCopyBufferToImage(cb, staging, gS2.cubeImg,
                               VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                               6, regions);

        // TRANSFER_DST -> SHADER_READ for all layers
        VkImageMemoryBarrier toShader;
        memset(&toShader, 0, sizeof(toShader));
        ...
        toShader.subresourceRange.layerCount     = 6;
        ...
        vkQueueSubmit(s1.vkQueue, 1, &s, VK_NULL_HANDLE);
        vkQueueWaitIdle(s1.vkQueue);
        vkFreeCommandBuffers(s1.vkDevice, s1.vkCommandPool, 1, &cb);
    }

    // 5) Create cube image view + sampler
    {
        VkImageViewCreateInfo vi;
        memset(&vi, 0, sizeof(vi));
        vi.sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO;
        vi.image = gS2.cubeImg;
        ...
        vr = vkCreateImageView(s1.vkDevice, &vi, NULL, &gS2.cubeView);
        if (vr) { fprintf(gpFile, "S2_CreateCubemap(): vkCreateImageView(cube) failed %d\n", vr); goto FAIL; }

        // Query features/properties for anisotropy
        VkPhysicalDeviceFeatures devFeat;
        memset(&devFeat, 0, sizeof(devFeat));
        vkGetPhysicalDeviceFeatures(s1.vkPhysicalDevice_selected, &devFeat);

        VkPhysicalDeviceProperties devProps;
        memset(&devProps, 0, sizeof(devProps));
        vkGetPhysicalDeviceProperties(s1.vkPhysicalDevice_selected, &devProps);
        ...
        si.mipLodBias = 0.0f;
        si.minLod     = 0.0f;
        si.maxLod     = 0.0f; // no mips
        if (devFeat.samplerAnisotropy) {
            float want = 16.0f, cap = devProps.limits.maxSamplerAnisotropy;
            si.anisotropyEnable = VK_TRUE;
            si.maxAnisotropy    = (want < cap) ? want : cap;
        } else {
        ...
        vr = vkCreateSampler(s1.vkDevice, &si, NULL, &gS2.cubeSampler);
        if (vr) { fprintf(gpFile, "S2_CreateCubemap(): vkCreateSampler(cube) failed %d\n", vr); goto FAIL; }
    }

    // 6) Cleanup staging + CPU pixels
    if (staging)    vkDestroyBuffer(s1.vkDevice, staging, NULL);
    if (stagingMem) vkFreeMemory(s1.vkDevice, stagingMem, NULL);
    for (int i = 0; i < 6; ++i) if (pix[i]) stbi_image_free(pix[i]);

    fprintf(gpFile, "S2_CreateCubemap(): OK (%dx%d) faces loaded\n", W, H);
    return VK_SUCCESS;
...
FAIL:
    ...
    if (stagingMem) vkFreeMemory(s1.vkDevice, stagingMem, NULL);
    for (int i = 0; i < 6; ++i) if (pix[i]) stbi_image_free(pix[i]);
    if (gS2.cubeView)    { vkDestroyImageView(s1.vkDevice, gS2.cubeView, NULL); gS2.cubeView = VK_NULL_HANDLE; }
    if (gS2.cubeSampler) { vkDestroySampler(s1.vkDevice, gS2.cubeSampler, NULL); gS2.cubeSampler = VK_NULL_HANDLE; }
    if (gS2.cubeImg)     { vkDestroyImage(s1.vkDevice, gS2.cubeImg, NULL); gS2.cubeImg = VK_NULL_HANDLE; }
    if (gS2.cubeMem)     { vkFreeMemory(s1.vkDevice, gS2.cubeMem, NULL); gS2.cubeMem = VK_NULL_HANDLE; }
    return vr;

FAIL_LOAD:
    for (int i = 0; i < 6; ++i) if (pix[i]) { stbi_image_free(pix[i]); pix[i] = NULL; }
    return vr;
}
...
VkResult S2_CreateDescriptorPool()
{
    ...
    ps[1].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER; ps[1].descriptorCount = 2; // cube + overlay

    VkDescriptorPoolCreateInfo ci{ VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO };
    ci.poolSizeCount = 2; ci.pPoolSizes = ps; ci.maxSets = 1;
    return vkCreateDescriptorPool(s1.vkDevice, &ci, nullptr, &gS2.descPool);
}

VkResult S2_CreateDescriptorSet()
{
    VkResult vr = VK_SUCCESS;

    VkDescriptorSetAllocateInfo ai{ VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO };
    ai.descriptorPool = gS2.descPool; ai.descriptorSetCount = 1; ai.pSetLayouts = &gS2.setLayout;
    vr = vkAllocateDescriptorSets(s1.vkDevice, &ai, &gS2.descSet); if(vr) return vr;

    VkDescriptorBufferInfo dbi{}; dbi.buffer = gS2.ubo; dbi.offset = 0; dbi.range = sizeof(Scene2_Resources::UBO);

    // Use whatever cubemap you created (see S2_CreateCubemap note above)
    VkDescriptorImageInfo sky{};  sky.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
    sky.imageView = gS2.cubeView ? gS2.cubeView : s1.vkImageView_texture; // fallback
    sky.sampler   = gS2.cubeSampler ? gS2.cubeSampler : s1.vkSampler_texture;

    // Overlay 0 (bind a placeholder if you don’t have images yet)
    VkDescriptorImageInfo ov{}; ov.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
    ov.imageView = (gS2.ovView[0] ? gS2.ovView[0] : s1.vkImageView_texture);
    ov.sampler   = (gS2.ovSampler[0] ? gS2.ovSampler[0] : s1.vkSampler_texture);

    VkWriteDescriptorSet w[3]{};
    w[0].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET; w[0].dstSet = gS2.descSet; w[0].dstBinding = 0;
    w[0].descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER; w[0].descriptorCount = 1; w[0].pBufferInfo = &dbi;

    w[1].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET; w[1].dstSet = gS2.descSet; w[1].dstBinding = 1;
    w[1].descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER; w[1].descriptorCount = 1; w[1].pImageInfo = &sky;

    w[2].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET; w[2].dstSet = gS2.descSet; w[2].dstBinding = 2;
    w[2].descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER; w[2].descriptorCount = 1; w[2].pImageInfo = &ov;

    vkUpdateDescriptorSets(s1.vkDevice, 3, w, 0, nullptr);
    gS2.ovBound = 0;
    return vr;
}
...
{
    if(index < 0) index = 0;
    if(index >= Scene2_Resources::kOverlayCount) index = Scene2_Resources::kOverlayCount - 1;
    if(gS2.ovView[index] == VK_NULL_HANDLE) return;

    VkDescriptorImageInfo di{}; di.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
    di.imageView = gS2.ovView[index]; di.sampler = gS2.ovSampler[index];

    VkWriteDescriptorSet w{ VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET };
    w.dstSet = gS2.descSet; w.dstBinding = 2;
    w.descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    w.descriptorCount = 1; w.pImageInfo = &di;
    vkUpdateDescriptorSets(s1.vkDevice, 1, &w, 0, nullptr);

    gS2.ovBound = index;
}

// ---------- Pipeline (targets Subpass #1) ----------
VkResult S2_CreatePipeline()
{
    VkResult vr = VK_SUCCESS;

    // Reuse your two VBs (pos @binding 0, uv @binding 1) and input state from Scene 1
    ...
    attrs[1].location = 1; attrs[1].binding = 1; attrs[1].format = VK_FORMAT_R32G32_SFLOAT;    attrs[1].offset = 0;
    ...
    vp.viewportCount = 1; vp.pViewports = &s1.vkViewport;
    vp.scissorCount  = 1; vp.pScissors  = &s1.vkRect2D_scissor;

    VkPipelineDepthStencilStateCreateInfo ds{ VK_STRUCTURE_TYPE_PIPELINE_DEPTH_STENCIL_STATE_CREATE_INFO };
    // Using Subpass 1 WITHOUT depth is fine; keep depth test off (simple skybox draw)
    ...
    stages[0].stage = VK_SHADER_STAGE_VERTEX_BIT; stages[0].module = gS2.vs; stages[0].pName = "main";
    stages[1].sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
    stages[1].stage = VK_SHADER_STAGE_FRAGMENT_BIT; stages[1].module = gS2.fs; stages[1].pName = "main";
    ...
    pci.layout              = gS2.pipelineLayout;
    pci.renderPass          = s1.vkRenderPass;
    pci.subpass             = 1; // <<< IMPORTANT: Subpass #1 >>>

    VkPipelineCacheCreateInfo pcci{ VK_STRUCTURE_TYPE_PIPELINE_CACHE_CREATE_INFO };
    VkPipelineCache cache = VK_NULL_HANDLE;
    vr = vkCreatePipelineCache(s1.vkDevice, &pcci, nullptr, &cache); if(vr) return vr;

    vr = vkCreateGraphicsPipelines(s1.vkDevice, cache, 1, &pci, nullptr, &gS2.pipeline);
    vkDestroyPipelineCache(s1.vkDevice, cache, nullptr);
    if(vr) { fprintf(gpFile, "S2_CreatePipeline(): vkCreateGraphicsPipelines failed %d\n", vr); }
    return vr;
}
...
static void S2_RecordSubpass1(VkCommandBuffer cb)
{
    vkCmdBindPipeline(cb, VK_PIPELINE_BIND_POINT_GRAPHICS, gS2.pipeline);

    // Bind your existing VBs (position binding=0, texcoord binding=1)
    VkBuffer pos = s1.vertexData_position.vkBuffer; VkDeviceSize off0 = 0;
    vkCmdBindVertexBuffers(cb, 0, 1, &pos, &off0);
    VkBuffer uv  = s1.vertexData_texcoord.vkBuffer; VkDeviceSize off1 = 0;
    vkCmdBindVertexBuffers(cb, 1, 1, &uv,  &off1);

    vkCmdBindDescriptorSets(cb, VK_PIPELINE_BIND_POINT_GRAPHICS, gS2.pipelineLayout, 0, 1, &gS2.descSet, 0, nullptr);
...
VkResult Initialize(void)
{
    ...
    // --- Scene 2 (Subpass 1) setup prototypes ---
    VkResult S2_CreateShaders(void);
    VkResult S2_CreateDescriptorSetLayout(void);
    VkResult S2_CreatePipelineLayout(void);
    VkResult S2_CreateUniformBuffer(void);
    VkResult S2_CreateDescriptorPool(void);
    VkResult S2_CreateDescriptorSet(void);
    VkResult S2_CreatePipeline(void);
    // Optional assets
    VkResult S2_CreateCubemap(const char* faces[6]);
    void     S2_BindOverlay(int index);
    ...
    */
    ...
    //checked conitions: [required format]
    ...
    //CreateTexture
    vkResult = createTexture("sky.png");
    if(vkResult != VK_SUCCESS)
    {
        fprintf(gpFile, "Initialize() --> createTexture() is failed %d\n", vkResult);
    }
    else
    {
        fprintf(gpFile, "Initialize() --> createTexture() is succedded\n");
    }
    ...
    // -------------------- Scene 2 (Subpass 1) boot --------------------
    // Load shaders + pipeline state for the Subpass 1 program
    vkResult = S2_CreateShaders();
    if(vkResult != VK_SUCCESS) { fprintf(gpFile, "Initialize() --> S2_CreateShaders() failed %d\n", vkResult); return vkResult; }

    vkResult = S2_CreateDescriptorSetLayout();
    if(vkResult != VK_SUCCESS) { fprintf(gpFile, "Initialize() --> S2_CreateDescriptorSetLayout() failed %d\n", vkResult); return vkResult; }

    vkResult = S2_CreatePipelineLayout();
    if(vkResult != VK_SUCCESS) { fprintf(gpFile, "Initialize() --> S2_CreatePipelineLayout() failed %d\n", vkResult); return vkResult; }

    vkResult = S2_CreateUniformBuffer();
    if(vkResult != VK_SUCCESS) { fprintf(gpFile, "Initialize() --> S2_CreateUniformBuffer() failed %d\n", vkResult); return vkResult; }

    // (Optional) If you have skybox faces, call S2_CreateCubemap(faces);
    // const char* faces[6] = {"right.png","left.png","top.png","bottom.png","front.png","back.png"};
    // S2_CreateCubemap(faces);

    vkResult = S2_CreateDescriptorPool();
    if(vkResult != VK_SUCCESS) { fprintf(gpFile, "Initialize() --> S2_CreateDescriptorPool() failed %d\n", vkResult); return vkResult; }

    vkResult = S2_CreateDescriptorSet();
    if(vkResult != VK_SUCCESS) { fprintf(gpFile, "Initialize() --> S2_CreateDescriptorSet() failed %d\n", vkResult); return vkResult; }

    vkResult = S2_CreatePipeline(); // uses s1.vkRenderPass, subpass=1
    if(vkResult != VK_SUCCESS) { fprintf(gpFile, "Initialize() --> S2_CreatePipeline() failed %d\n", vkResult); return vkResult; }
    ...
    //DefaultClearColor
    memset(&s1.vkClearColorValue, 0, sizeof(s1.vkClearColorValue));
    s1.vkClearColorValue.float32[0] = 0.0f;
    s1.vkClearColorValue.float32[1] = 0.0f;
    s1.vkClearColorValue.float32[2] = 0.0f;
    s1.vkClearColorValue.float32[3] = 1.0f;
    //DefaultClearDepthStencil
    memset(&s1.vkClearDepthStencilValue, 0, sizeof(s1.vkClearDepthStencilValue));
...
VkResult createTexture(const char* textureFileName)
{
    ...
    if(fp == NULL)
    {
        fprintf(gpFile, "createTexture() --> fOpen() failed to open %s texture file\n", textureFileName);
...
VkResult buildCommandBuffers(void)
{
    ...
    for (uint32_t i = 0; i < s1.swapchainImageCount; i++)
    {
        ...
        if (vkResult != VK_SUCCESS) { fprintf(gpFile, "buildComm...ommandBuffer failed %u (%d)\n", i, vkResult); return vkResult; }
        ...
        // ---- SUBPASS 0 : Scene 1 into Color A ----
        ...
        // ---- Move to SUBPASS 1 (composite to swapchain) ----
        vkCmdNextSubpass(s1.vkCommandBuffer_array[i], VK_SUBPASS_CONTENTS_INLINE);

        // === Subpass 1: draw Scene 2 (VK_s2) to swapchain ===
        vkCmdBindPipeline(s1.vkCommandBuffer_array[i], VK_PIPELINE_BIND_POINT_GRAPHICS, gS2.pipeline);

        // Descriptor set for Scene 2
        vkCmdBindDescriptorSets(s1.vkCommandBuffer_array[i],
                                VK_PIPELINE_BIND_POINT_GRAPHICS,
                                gS2.pipelineLayout,
                                0, 1, &gS2.descSet, 0, NULL);

        // Bind the same cube vertex buffers (binding 0: position, 1: texcoord)
        {
            VkBuffer posBufs2[1] = { s1.vertexData_position.vkBuffer };
            VkDeviceSize posOffs2[1] = { 0 };
            vkCmdBindVertexBuffers(s1.vkCommandBuffer_array[i], 0, 1, posBufs2, posOffs2);

            VkBuffer texBufs2[1] = { s1.vertexData_texcoord.vkBuffer };
            VkDeviceSize texOffs2[1] = { 0 };
            vkCmdBindVertexBuffers(s1.vkCommandBuffer_array[i], 1, 1, texBufs2, texOffs2);
        }
...
{
    // ---------------------------------------------------------------------
    // 0) Load pixels (force RGBA8)
    // ---------------------------------------------------------------------
    int w = 0, h = 0, ch = 0;
    stbi_uc* pixels = stbi_load(path, &w, &h, &ch, STBI_rgb_alpha);
    if (!pixels || w <= 0 || h <= 0) {
        if (pixels) stbi_image_free(pixels);
        fprintf(gpFile, "createTexture2D(): stbi_load failed for %s\n", path ? path : "(null)");
        return VK_ERROR_INITIALIZATION_FAILED;
    }
    VkDeviceSize imageSize = (VkDeviceSize)w * (VkDeviceSize)h * 4;

    // Desired mip levels (fallback to 1 if linear blit not supported)
    uint32_t mipLevels = 1;
    {
        uint32_t maxSide = (w > h) ? (uint32_t)w : (uint32_t)h;
        while (maxSide > 1) { maxSide >>= 1; ++mipLevels; }

        VkFormatProperties fp;
        memset(&fp, 0, sizeof(fp));
        vkGetPhysicalDeviceFormatProperties(s1.vkPhysicalDevice_selected,
                                            VK_FORMAT_R8G8B8A8_UNORM, &fp);
        if ((fp.optimalTilingFeatures & VK_FORMAT_FEATURE_SAMPLED_IMAGE_FILTER_LINEAR_BIT) == 0) {
            mipLevels = 1;
            fprintf(gpFile, "createTexture2D(): linear blit unsupported; using 1 mip level\n");
        }
    }

    // Objects (init to NULL for safe cleanup on FAIL)
    VkBuffer       staging            = VK_NULL_HANDLE;
    VkDeviceMemory stagingMem         = VK_NULL_HANDLE;
    ...
    // ---------------------------------------------------------------------
    {
        ...
        vr = vkCreateBuffer(s1.vkDevice, &ci, NULL, &staging);
        if (vr) { fprintf(gpFile, "createTexture2D(): vkCreateBuffer(staging) failed %d\n", vr); goto FAIL; }
        ...
        vr = vkAllocateMemory(s1.vkDevice, &ai, NULL, &stagingMem);
        if (vr) { fprintf(gpFile, "createTexture2D(): vkAllocateMemory(staging) failed %d\n", vr); goto FAIL; }
        vr = vkBindBufferMemory(s1.vkDevice, staging, stagingMem, 0);
        if (vr) { fprintf(gpFile, "createTexture2D(): vkBindBufferMemory(staging) failed %d\n", vr); goto FAIL; }

        void* p = NULL;
        vr = vkMapMemory(s1.vkDevice, stagingMem, 0, imageSize, 0, &p);
        if (vr) { fprintf(gpFile, "createTexture2D(): vkMapMemory(staging) failed %d\n", vr); goto FAIL; }
        memcpy(p, pixels, (size_t)imageSize);
        vkUnmapMemory(s1.vkDevice, stagingMem);
    }

    // ---------------------------------------------------------------------
    // 2) Optimal-tiled image (DEVICE_LOCAL)
    // ---------------------------------------------------------------------
    {
        VkImageCreateInfo ci;
        memset(&ci, 0, sizeof(ci));
        ...
        vr = vkCreateImage(s1.vkDevice, &ci, NULL, &image);
        if (vr) { fprintf(gpFile, "createTexture2D(): vkCreateImage failed %d\n", vr); goto FAIL; }

        VkMemoryRequirements mr;
        memset(&mr, 0, sizeof(mr));
        vkGetImageMemoryRequirements(s1.vkDevice, image, &mr);
        ...
        vr = vkAllocateMemory(s1.vkDevice, &ai, NULL, &imageMem);
        if (vr) { fprintf(gpFile, "createTexture2D(): vkAllocateMemory(image) failed %d\n", vr); goto FAIL; }
        vr = vkBindImageMemory(s1.vkDevice, image, imageMem, 0);
        if (vr) { fprintf(gpFile, "createTexture2D(): vkBindImageMemory failed %d\n", vr); goto FAIL; }
    }

    // ---------------------------------------------------------------------
    // 3) Layout: UNDEFINED -> TRANSFER_DST_OPTIMAL + copy buffer->image
    // ---------------------------------------------------------------------
    {
        // Allocate one-time CB
        VkCommandBufferAllocateInfo a;
        memset(&a, 0, sizeof(a));
        a.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
        a.commandPool = s1.vkCommandPool;
        ...
        VkCommandBuffer cb = VK_NULL_HANDLE;
        vr = vkAllocateCommandBuffers(s1.vkDevice, &a, &cb);
        if (vr) { fprintf(gpFile, "createTexture2D(): vkAllocateCommandBuffers(1) failed %d\n", vr); goto FAIL; }
        ...
        // Barrier: all mips to TRANSFER_DST
    ...
    // ---------------------------------------------------------------------
    // 4) Mipmap generation (if any) OR final transition to SHADER_READ
    // ---------------------------------------------------------------------
    if (mipLevels > 1) {
        int32_t mipW = w;
        int32_t mipH = h;
        ...
        VkCommandBuffer cb = VK_NULL_HANDLE;
        vr = vkAllocateCommandBuffers(s1.vkDevice, &a, &cb);
        if (vr) { fprintf(gpFile, "createTexture2D(): vkAllocateCommandBuffers(2) failed %d\n", vr); goto FAIL; }
        ...
        for (uint32_t i = 1; i < mipLevels; ++i) {
            ...
            blit.srcOffsets[0].x = 0;      blit.srcOffsets[0].y = 0;      blit.srcOffsets[0].z = 0;
            blit.srcOffsets[1].x = mipW;   blit.srcOffsets[1].y = mipH;   blit.srcOffsets[1].z = 1;
            ...
            blit.dstOffsets[0].x = 0;      blit.dstOffsets[0].y = 0;      blit.dstOffsets[0].z = 0;
            blit.dstOffsets[1].x = nextW;  blit.dstOffsets[1].y = nextH;  blit.dstOffsets[1].z = 1;
        ...
        memset(&lastToShader, 0, sizeof(lastToShader));
        ...
        lastToShader.image = image;
        lastToShader.subresourceRange.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
        lastToShader.subresourceRange.baseMipLevel = mipLevels - 1;
        lastToShader.subresourceRange.levelCount   = 1;
        lastToShader.subresourceRange.baseArrayLayer = 0;
        lastToShader.subresourceRange.layerCount     = 1;

        vkCmdPipelineBarrier(cb,
                             VK_PIPELINE_STAGE_TRANSFER_BIT,
                             VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT,
                             0, 0, NULL, 0, NULL, 1, &lastToShader);
        ...
        // No mips: TRANSFER_DST -> SHADER_READ
        VkCommandBufferAllocateInfo a;
        memset(&a, 0, sizeof(a));
        a.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
        a.commandPool = s1.vkCommandPool;
        ...
        VkCommandBuffer cb = VK_NULL_HANDLE;
        vr = vkAllocateCommandBuffers(s1.vkDevice, &a, &cb);
        if (vr) { fprintf(gpFile, "createTexture2D(): vkAllocateCommandBuffers(3) failed %d\n", vr); goto FAIL; }
    ...
    // ---------------------------------------------------------------------
    {
        ...
        vr = vkCreateImageView(s1.vkDevice, &vi, NULL, &view);
        if (vr) { fprintf(gpFile, "createTexture2D(): vkCreateImageView failed %d\n", vr); goto FAIL; }
        ...
        si.mipLodBias = 0.0f;
        si.minLod     = 0.0f;
        si.maxLod     = (float)(mipLevels - 1);
        if (devFeat.samplerAnisotropy) {
            float want = 16.0f;
            float cap  = devProps.limits.maxSamplerAnisotropy;
            si.anisotropyEnable = VK_TRUE;
            si.maxAnisotropy    = (want < cap) ? want : cap;
        } else {
            si.anisotropyEnable = VK_FALSE;
        ...
        vr = vkCreateSampler(s1.vkDevice, &si, NULL, &samp);
        if (vr) { fprintf(gpFile, "createTexture2D(): vkCreateSampler failed %d\n", vr); goto FAIL; }
    }

    // ---------------------------------------------------------------------
    // Success: set outputs, cleanup staging, free pixels
    // ---------------------------------------------------------------------
    *outImg     = image;
    *outMem     = imageMem;
    *outView    = view;
    *outSampler = samp;
    ...
    fprintf(gpFile, "createTexture2D(): OK (%dx%d, mips=%u) file=%s\n", w, h, mipLevels, path);
    return VK_SUCCESS;

FAIL:
    if (pixels)      stbi_image_free(pixels);
    if (view)        vkDestroyImageView(s1.vkDevice, view, NULL);
    if (samp)        vkDestroySampler(s1.vkDevice, samp, NULL);
    if (image)       vkDestroyImage(s1.vkDevice, image, NULL);
    if (imageMem)    vkFreeMemory(s1.vkDevice, imageMem, NULL);
    if (staging)     vkDestroyBuffer(s1.vkDevice, staging, NULL);
    if (stagingMem)  vkFreeMemory(s1.vkDevice, stagingMem, NULL);
    fprintf(gpFile, "createTexture2D(): FAILED for %s\n", path ? path : "(null)");
    return (vr ? vr : VK_ERROR_UNKNOWN);
}
