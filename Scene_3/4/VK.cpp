...
VkResult updateUniformBuffer(void)
{
    ...
    const bool  kFreezeOrbits        = true;   // keep equal spacing stable (set false to orbit)
    ...
    // Big, designer planet radii (world units) — tweak freely
    const float planetBase[8] = { 260.0f, 260.0f, 260.0f, 260.0f, 260.0f, 260.0f, 260.0f, 260.0f };

    // Projected Sun radius in NDC (0..1). Controls how tall the visible half looks.
    const float kSunNdcRadius        = 0.35f;

    // Target equal gap between planets (edge-to-edge) in NDC
    const float kTargetGapNDC        = 0.06f;

    // ===================== PROJECTION (P00 only needs fovy/aspect) =====================
    ...
    // ===================== SUN: center at x_ndc = -1 (exactly half visible) =====================
    const float kSunScale   = kSunBase * kSunScaleGlobal;

    // Choose depth so projected radius == kSunNdcRadius
    const float sunZ        = -(kSunScale * P00) / kSunNdcRadius;

    // Center x_ndc = -1.0 -> exactly half the Sun is visible on the right
    const float sunCenterNdc = -1.0f;
    const float sunX         = sunCenterNdc * (-sunZ) / P00;

    const glm::vec3 SUN_POS(sunX, 0.0f, sunZ);

    // Visible right rim of the Sun (since radius = kSunNdcRadius)
    const float sunRightNdc  = -1.0f + kSunNdcRadius;

    // NDC<->world scale at sunZ
    const float ndc_to_world = (-sunZ) / P00;
    const float world_to_ndc = 1.0f / ndc_to_world;

    // ===================== PLANET SIZE AUTO-FIT ACROSS VIEWPORT =====================
    float kScale[8], maxPlanet = 0.0f;
    for (int i = 0; i < 8; ++i) { kScale[i] = planetBase[i] * kPlanetScaleGlobal; maxPlanet = glm::max(maxPlanet, kScale[i]); }

    // Convert those radii to NDC at z = sunZ
    float r_ndc[8], sumDiamNdc = 0.0f;
    for (int i = 0; i < 8; ++i) { r_ndc[i] = kScale[i] * world_to_ndc; sumDiamNdc += 2.0f * r_ndc[i]; }

    // Planets fill entire region to the right of the visible Sun half
    const int   N            = 8;
    const float xStart       = sunRightNdc; // start just to the right of visible Sun half
    const float xEnd         = 1.0f;
    const float regionWidth  = xEnd - xStart;

    // Equal gaps (N+1 gaps including left/right margins)
    float g = kTargetGapNDC;
    float availForPlanets = regionWidth - (N + 1) * g;

    if (availForPlanets <= 0.0f) {
        g = glm::max(0.0f, regionWidth / float(N + 1));
        availForPlanets = 0.0f;
    }

    float scaleNdc = (sumDiamNdc > 0.0f) ? (availForPlanets / sumDiamNdc) : 1.0f;
    scaleNdc = glm::clamp(scaleNdc, 0.05f, 10.0f);

    // Apply scale and recompute exact gap so it fits perfectly
    float sumDiamNdcScaled = 0.0f;
    for (int i = 0; i < 8; ++i) { r_ndc[i] *= scaleNdc; kScale[i] = r_ndc[i] * ndc_to_world; sumDiamNdcScaled += 2.0f * r_ndc[i]; }
    g = (regionWidth - sumDiamNdcScaled) / float(N + 1);

    // Centers in NDC with equal edge-to-edge gaps
    float centerNdc[8];
    float x = xStart + g + r_ndc[0];
    centerNdc[0] = x;
    for (int i = 1; i < 8; ++i) { x += r_ndc[i - 1] + g + r_ndc[i]; centerNdc[i] = x; }

    // Convert NDC centers to world X, then orbit radii about SUN_POS
    float orbitRadius[8];
    for (int i = 0; i < 8; ++i) {
        const float cx_world = centerNdc[i] * ndc_to_world;
        orbitRadius[i] = cx_world - SUN_POS.x;
    }

    // ===================== FINAL PROJECTION WITH SAFE FAR =====================
    const float farPlane = fabsf(sunZ) + 4.0f * (kSunScale + maxPlanet) + 500.0f;
