...
VkResult updateUniformBuffer(void)
{
    ...
    // -------------------- SIZE CONTROL --------------------
    // Make Earth bigger/smaller here (visual world radius).
    const float kEarth = 100.0f;  // tweak to taste

    // Planet radii as multiples of Earth (≈ real-world-ish ratios, with Earth boosted as requested).
    //            Merc    Ven     Earth   Mars    Jup     Sat     Ur      Nep
    const float baseRatio[8] = { 0.383f, 0.949f, 20.000f, 0.532f, 11.21f, 9.45f, 4.01f, 3.88f };

    float kScale[8];
    for (int i = 0; i < 8; ++i) kScale[i] = baseRatio[i] * kEarth;

    // Sun kept visually large (sliver like your mock). Increase if you want a fatter Sun.
    const float kSunScale = 17.5f * kEarth;

    // Constant surface-to-surface clearance so neighbors don't touch.
    const float kClear = 0.60f;

    // -------------------- EQUAL ORBIT SPACING --------------------
    // One fixed center-to-center STEP large enough for EVERY adjacent pair (incl. Sun↔Mercury).
    float step = kSunScale + kScale[0] + kClear; // Sun ↔ Mercury candidate
    for (int i = 0; i < 7; ++i)
        step = glm::max(step, kScale[i] + kScale[i + 1] + kClear);

    // Concentric radii for Mercury..Neptune with equal center spacing:
    // r[0] = 1*step, r[1] = 2*step, ..., r[7] = 8*step
    float r[8];
    for (int i = 0; i < 8; ++i) r[i] = (i + 1) * step;

    // Keep everything in front of the camera even when planets swing to +90°
    const float sunZ = -(r[7] + 1.0f);  // small margin

    // -------------------- PROJECTION --------------------
    const float fovy   = glm::radians(45.0f);
    const float aspect = (float)winWidth / (float)winHeight;
    const float farPlane = glm::max(100.0f, 2.2f * r[7] + kSunScale + 10.0f);

    glm::mat4 projection = glm::perspective(fovy, aspect, 0.1f, farPlane);
    projection[1][1] *= -1.0f;

    // -------------------- SUN AT LEFT EDGE (−X) --------------------
    // For center (x,z): x_ndc = (x * P00)/(-z); projected radius r_ndc = (R * P00)/(-z).
    // Put LEFT rim at x_ndc = -1  ⇒  center at x_ndc = -1 + r_ndc.
    const float P00          = projection[0][0];
    const float r_ndc        = (kSunScale * P00) / (-sunZ);
    const float x_ndc_center = -1.0f + r_ndc;           // touch the border
    const float sunX         = x_ndc_center * (-sunZ) / P00;

    const glm::vec3 SUN_POS(sunX, 0.0f, sunZ);
    const float     PHASE_RAD = glm::radians(-90.0f);   // planets start from behind the Sun
    ...
    // Helper: model matrix for a planet on a circular orbit around the Sun
    auto planetModel = [&](float angleDeg, float radius, float scale)->glm::mat4 {
        const float theta   = glm::radians(angleDeg) + PHASE_RAD; // start from behind
        const glm::vec3 off = { radius * cosf(theta), 0.0f, radius * sinf(theta) };
        ...
        const glm::mat4 Ry  = glm::rotate  (glm::mat4(1.0f), glm::radians(angleDeg), glm::vec3(0.0f, 1.0f, 0.0f));
        return T0 * Tor * S * Ry;
    };
