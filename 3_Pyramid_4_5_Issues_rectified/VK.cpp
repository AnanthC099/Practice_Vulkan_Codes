/* =========================================================================
 *  Vk.cpp – single‑file Vulkan sample with **depth buffering**
 *  Rotating pyramid, per‑vertex colour, push‑constant MVP.
 *  Depth buffer: runtime‑selected format via dynamic rendering.
 * ========================================================================= */
...
/* --- new forward decls (depth / helpers) --- */
...
/* -------- depth globals -------- */
static VkFormat       vkFormat_depth   = VK_FORMAT_UNDEFINED;
static bool           gDepthHasStencil = false;
...
static const Vertex gPyramidVertices[] =
{
    /* front face – red */
...
static const uint32_t gVertexCount =
        sizeof(gPyramidVertices)/sizeof(Vertex);
...
LRESULT CALLBACK WndProc(HWND hwnd,UINT iMsg,WPARAM wParam,LPARAM lParam)
{
    switch(iMsg)
    {
        ...
        case WM_CHAR:
            if(wParam=='f'||wParam=='F'){ LOG("Toggle full‑screen"); ToggleFullscreen(); gbFullscreen=!gbFullscreen; }
            break;

        case WM_KEYDOWN:
            if(wParam==VK_ESCAPE){ LOG("ESC pressed – closing"); DestroyWindow(hwnd); }
            break;

        case WM_CLOSE:   LOG("WM_CLOSE received");   DestroyWindow(hwnd); break;
        case WM_DESTROY: LOG("WM_DESTROY received – posting quit"); PostQuitMessage(0); break;
    }
...
/* =================================================== */
/*   SINGLE‑TIME COMMAND HELPERS (layout transitions)  */
/* =================================================== */
static VkCommandBuffer BeginSingleTimeCommands(void)
{
...
static VkResult CreateVulkanInstance(void)
{
    ...
    if(instLayerCount){
        ...
        VK_CHECK(CreateDebugUtilsMessengerEXT(
            vkInstance,&dbg,NULL,&gDebugUtilsMessenger));
    }
    return VK_SUCCESS;
}
...
static VkResult CreateVertexBuffer(void)
{
    ...
    bci.size        = sizeof(gPyramidVertices);
    ...
    memcpy(data,gPyramidVertices,sizeof(gPyramidVertices));
...
static VkResult CreateGraphicsPipeline(void)
{
    ...
    pr.depthAttachmentFormat   = vkFormat_depth;   /* portable depth format */
...
/* =================================================== */
/*       DEPTH FORMAT PICKING  (portable, robust)      */
/* =================================================== */
static VkFormat PickDepthFormat(void)
{
    const VkFormat cand[] = {
    ...
    for(auto f : cand){
        ...
        vkGetPhysicalDeviceFormatProperties(
            vkPhysicalDevice_selected,f,&props);
        if(props.optimalTilingFeatures &
           VK_FORMAT_FEATURE_DEPTH_STENCIL_ATTACHMENT_BIT){
            gDepthHasStencil = (f == VK_FORMAT_D24_UNORM_S8_UINT);
            LOG("Picked depth format 0x%x (stencil=%s)", f,
                gDepthHasStencil?"yes":"no");
            return f;
        }
    }
...
/* =================================================== */
static void TransitionDepthImage(void)
{
...
static VkResult CreateDepthResources(void)
{
    ...
    /* 3. One‑time transition UNDEFINED → DEPTH_ATTACHMENT_OPTIMAL */
...
static VkResult recordCommandBuffer(uint32_t imgIndex,float angle)
{
    ...
    //proj[1][1]*=-1.0f;  /* already flipped by negative viewport height */
    ...
    toPresent.srcAccessMask = VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT; /* explicit */
...
static VkResult recreateSwapchain(void)
{
    ...
    VK_CHECK(CreateCommandPoolBuffers());   /* command pool before depth */
...
static VkResult initialize(void)
{
    ...
    if(vkFormat_depth==VK_FORMAT_UNDEFINED){
        ERR("No supported depth format found");
        return VK_ERROR_FORMAT_NOT_SUPPORTED;
    }
    VK_CHECK(CreateDeviceQueue());
    VK_CHECK(CreateSwapchain());
    VK_CHECK(CreateImagesViews());
    VK_CHECK(CreateCommandPoolBuffers());  /* pool before first depth image */
    VK_CHECK(CreateDepthResources());
