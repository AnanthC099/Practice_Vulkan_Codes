...
VkResult updateUniformBuffer(void)
{
    ...
    // ----- GLOBAL SCALE (keep this as your master knob) -----
    // This is the “Earth radius in world units”. Tweak ONLY this to scale everything up/down.
    const float kEarth = 50.0f;

    // ----- REAL RADIUS RATIOS (planet radius / Earth radius) -----
    //            Merc    Ven     Earth   Mars    Jup     Sat     Ur      Nep
    const float planetToEarth[8] = { 100.0f, 100.0f, 100.000f, 100.0f, 100.0f, 100.0f, 100.0f, 100.0f };

    // Convert to world scales using your kEarth
    float kScale[8];
    for (int i = 0; i < 8; ++i) kScale[i] = planetToEarth[i] * kEarth;

    // Sun uses real ratio too (≈109.1 × Earth radius)
    const float kSunScale = 109.1f * kEarth;

    // Small gap so neighbors never touch (tune if needed)
    const float kClear = 0.90f;

    // ----- EQUAL ORBIT SPACING (center-to-center), big enough for every adjacent pair -----
    float step = kSunScale + kScale[0] + kClear;   // Sun ↔ Mercury
    for (int i = 0; i < 7; ++i)
        step = glm::max(step, kScale[i] + kScale[i + 1] + kClear);

    // Radii of Mercury..Neptune: r[i] = (i+1) * step
    float r[8];
    for (int i = 0; i < 8; ++i) r[i] = (i + 1) * step;

    // Keep everything in front and comfortably inside the far plane
    const float sunZ = -(r[7] + 1.0f);
    ...
    // ----- SUN AT LEFT EDGE (−X) -----
    // Left rim at x_ndc = -1 ⇒ center x_ndc = -1 + r_ndc, where r_ndc = (R * P00)/(-z)
    const float P00          = projection[0][0];
    const float r_ndc        = (kSunScale * P00) / (-sunZ);
    const float x_ndc_center = -1.0f + r_ndc;
    const float sunX         = x_ndc_center * (-sunZ) / P00;

    const glm::vec3 SUN_POS(sunX, 0.0f, sunZ);

    // Start planets “from behind the Sun”
    const float PHASE_RAD = glm::radians(-90.0f);
    ...
    // Helper: circular orbit around SUN_POS in XZ plane
    auto planetModel = [&](float angleDeg, float radius, float scale)->glm::mat4 {
        const float theta   = glm::radians(angleDeg) + PHASE_RAD;
    ...
    // ----- PLANETS -----
    {
        ...
        u.modelMatrix = planetModel(angle_earth,   r[2], kScale[2]); // Earth = 1.0 × kEarth
