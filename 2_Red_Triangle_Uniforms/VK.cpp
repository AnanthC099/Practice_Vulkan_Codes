 ...
 *  now uses a uniform buffer (descriptor set 0 / binding 0) for colour
 *  and a push constant for the projection matrix
 * ========================================================================= */
...
/* ---------- GLM (for the perspective matrix) ---------- */
#include "glm/glm.hpp"
...
/* --- new for colour‑UBO ------------------------------------------------- */
static VkResult CreateColorUBO(void);
static VkResult CreateDescriptorStuff(void);
...
/* --- descriptor / UBO objects for colour ------------------------------- */
static VkDescriptorSetLayout gDescSetLayout = VK_NULL_HANDLE;
static VkDescriptorPool      gDescPool      = VK_NULL_HANDLE;
static VkDescriptorSet       gDescSet       = VK_NULL_HANDLE;
static VkBuffer              gColorUBO      = VK_NULL_HANDLE;
static VkDeviceMemory        gColorMemory   = VK_NULL_HANDLE;
...
/* =================================================== */
/*          COLOUR UBO & DESCRIPTOR‑SET OBJECTS        */
/* =================================================== */
static VkResult CreateColorUBO(void)
{
    LOG("CreateColorUBO()");
    VkBufferCreateInfo bci{ VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO };
    bci.size  = sizeof(float)*4;                    // one vec4
    bci.usage = VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT;
    bci.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
    VK_CHECK(vkCreateBuffer(vkDevice,&bci,nullptr,&gColorUBO));

    VkMemoryRequirements req;
    vkGetBufferMemoryRequirements(vkDevice,gColorUBO,&req);
    ...
    VK_CHECK(vkAllocateMemory(vkDevice,&mai,nullptr,&gColorMemory));
    vkBindBufferMemory(vkDevice,gColorUBO,gColorMemory,0);

    /* write constant red */
    const float red[4] = {1.0f,0.0f,0.0f,1.0f};
    void* ptr=nullptr;
    vkMapMemory(vkDevice,gColorMemory,0,sizeof(red),0,&ptr);
    memcpy(ptr,red,sizeof(red));
    vkUnmapMemory(vkDevice,gColorMemory);
    return VK_SUCCESS;
}

static VkResult CreateDescriptorStuff(void)
{
    LOG("CreateDescriptorStuff()");
    /* --- layout -------------------------------------------------------- */
    ...
    dslci.pBindings    = &ub;
    VK_CHECK(vkCreateDescriptorSetLayout(vkDevice,&dslci,nullptr,&gDescSetLayout));
    ...
    dpci.pPoolSizes    = &ps;
    VK_CHECK(vkCreateDescriptorPool(vkDevice,&dpci,nullptr,&gDescPool));
    ...
    VK_CHECK(vkAllocateDescriptorSets(vkDevice,&dsai,&gDescSet));

    VkDescriptorBufferInfo dbi{};
    dbi.buffer = gColorUBO;
    dbi.offset = 0;
    dbi.range  = sizeof(float)*4;
...
static VkResult CreatePipelineLayout(void)
{
    ...
    /* push‑constant range: 64 bytes for the mat4 */
    ...
    plci.pSetLayouts            = &gDescSetLayout;      // ← NEW
    plci.pushConstantRangeCount = 1;
    plci.pPushConstantRanges    = &pc;
    VK_CHECK(vkCreatePipelineLayout(vkDevice,&plci,nullptr,&gPipelineLayout));
    return VK_SUCCESS;
}
...
static VkResult CreateGraphicsPipeline(void)
{
    ...
    VkShaderModule vert = CreateShaderModule("./triangle.vert.spv");
    VkShaderModule frag = CreateShaderModule("./triangle.frag.spv");
...
static VkResult buildCommandBuffers(void)
{
    ...
    for(uint32_t i=0;i<swapchainImageCount;i++){
        ...
        /* Bind pipeline / descriptor set / vertex buffer */
        vkCmdBindPipeline(vkCommandBuffer_array[i],VK_PIPELINE_BIND_POINT_GRAPHICS,gPipeline);
        vkCmdBindDescriptorSets(
            vkCommandBuffer_array[i], VK_PIPELINE_BIND_POINT_GRAPHICS,
            gPipelineLayout, 0, 1, &gDescSet, 0, nullptr);
        ...
        proj[1][1]*=-1.0f;
        vkCmdPushConstants(
            vkCommandBuffer_array[i], gPipelineLayout, VK_SHADER_STAGE_VERTEX_BIT,
            0, sizeof(glm::mat4), &proj[0][0]);
...
static VkResult initialize(void)
{
    ...
    /* uniforms & descriptors BEFORE pipeline objects */
    VK_CHECK(CreateColorUBO());
    VK_CHECK(CreateDescriptorStuff());
...
static void uninitialize(void)
{
    ...
    if(vkDevice){
        ...
        /* destroy colour descriptor/UBO resources */
        if(gDescPool)          vkDestroyDescriptorPool(vkDevice,gDescPool,nullptr);
        if(gDescSetLayout)     vkDestroyDescriptorSetLayout(vkDevice,gDescSetLayout,nullptr);
        if(gColorUBO)          vkDestroyBuffer(vkDevice,gColorUBO,nullptr);
        if(gColorMemory)       vkFreeMemory(vkDevice,gColorMemory,nullptr);
...
int WINAPI WinMain(HINSTANCE hInst,HINSTANCE hPrev,LPSTR lpszCmdLine,int iCmdShow)
{
    ...
    ghwnd=CreateWindowEx(
        ...
        TEXT("VulkanApp – Perspective Triangle (UBO colour)"),
